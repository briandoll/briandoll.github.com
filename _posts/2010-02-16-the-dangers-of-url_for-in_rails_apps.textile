...
# A Rails app that generating some links with full URL named routes
...
<pre>
 ...
 10.0.0.10  emphaticsolutions.com
</pre>

Now, when we visit the target website, those URLs look like this:
...
If the target application is caching content that includes these links, and you're lucky enough to hit an uncached section of the site, you've just injected your own hostname into the target's website links for other users to see and click on.

h3. Lots of stars must align

Sure, there are a lot of ifs that have to pan out for this issue to be exploitable.  That doesn't make the issue any less real.  One solution to this would be to include a configuration option that is defined per environment, to identify the desired hostname for a given app. I think this solution works for most applications, and if you don't use the configuration, you'll get the existing behavior.

Another solution is to configure Apache to use a specific hostname for both http and https URLs:
